    ...
    engine {
        addHandler { request ->
            ...
            when {
                ...
                requestUrl.startsWith("/${environment.clients.dialogmeldingpdfgen.baseUrl}") ->
                    pdfGenClientMockResponse(request)

                requestUrl.startsWith("/${environment.clients.padm2.baseUrl}") ->
                    padm2ClientMockResponse(request)

                requestUrl.startsWith("/${environment.clients.oppfolgingstilfelle.baseUrl}") ->
                    oppfolgingstilfelleClientMockResponse(request)
